...
async function ifAddressNotInDatabase(address){
  ...
  if(!isAddressIn){
    $('.profileGenerated').html(`<p id="notYetConnectedText">This address has not yet connected their wallet to Crypto<span class="gradient-text">R<span></p>`);
  }
};
...
function twitterShareProfile(){
  ...
  let tweet = `https://twitter.com/intent/tweet?text=Check%20out%20this%20artist%20on%20CryptoR!&hashtags=CryptoR%2Cbsc%2Cnonfungible%2Cdigitalasset%2Cnft&via=CryptoR&url=${page}`;
  $('#twitterShare').click(()=>{
...
function facebookShareProfile(username){
  ...
  let height = screen.height / 3;
  let page = window.location.href;
  let post = `https://www.facebook.com/sharer/sharer.php?u=${page}%3F&quote=Check%20out%20${username}'s%20NFTs%20on%20CryptoR`;
  $('#facebookShare').click(()=>{
...
function emailShareProfile(username){
  let page = window.location.href;
  $('#emailShare').click(()=>{
    window.location.href = `mailto:user@example.com?subject=${username}'s%20profile%20on%20CryptoR&body=Check%20out%20the%20artwork%20of%20${username}%20on%20CryptoR,%20${page}`;
  });
};
                    ...
                    <div id="card`+tokenAddress+id+`" class="card shadow-sm">
                      <div class="top-row">
                        <div class="creator-photo">
                          <a id='owner`+tokenAddress+id+`' href="http://localhost:8000/client/profile.html?address=`+owner+`"><img class="owner shadow-sm" id="ownerPhoto`+tokenAddress+id+`" src="" width="40" alt="owner photo">
...
function userCard(ethAddress){
  let userCard = `<li id="userItem`+ethAddress+`" class="list-group-item d-flex justify-content-between align-items-center">
                    <a href="http://localhost:8000/client/profile.html?address=`+ethAddress+`">
