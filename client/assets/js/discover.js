...
$('#forSale').click(async ()=>{
  ...
  let activeCount = ifOfferDetailsDuplicatesRemoved.filter(item => !item.isSold && item.active).length;
  if(activeCount < 1){
    $('.minted-wrapper').html(`<div class="no-art-for-sale">There is currently no artwork for sale on CryptoR, but you can change that <a class="gradient-text" href="create.html"> here!<a> 😎<div>`);
  }
  recentlyPutForSale();
});
...
$('#notForSale').click(async ()=>{
  ...
  let inactiveCount = inactiveArtwork.filter(item => !item.active).length;
  if(inactiveCount < 1){
    $('.minted-wrapper').html(`<div class="no-art-for-sale">There is currently no artwork not for sale on CryptoR, but you can change that <a class="gradient-text" href="create.html"> here!<a> 😎<div>`);
  }
...
async function viewAll(){
  ...
  if(activeCount < 1 && inactiveCount < 1){
    $('.minted-wrapper').html(`<div class="no-art-for-sale">There is currently no artwork on CryptoR, but you can change that <a class="gradient-text" href="create.html"> here!<a> 😎<div>`);
  }
  recentlyPutForSale();
  recentlyMintedAndNotOnSale();
};
...
async function recentlySold(){
  try {
    ...
    for (i = 0; i < restrictedToRecentEightSalesArray.length; i++) {
      ...
      $('#soldCardNotForSale' + tokenAddress + id).html(`Sold For: <br><span class="for-sale-text">${priceInEth} AVAX</span>`);
  ...
  try{
    ...
    for (i = 0; i < ifOfferDetailsLength; i++) {
      if(ifOfferDetailsDuplicatesRemoved[i].active == true && ifOfferDetailsDuplicatesRemoved[i].isSold == false){
        ...
        let priceInEth = web3.utils.fromWei(price, 'ether');
        $('#forSale' + tokenAddress + id).html(`<span class="for-sale-text">${priceInEth} AVAX</span>`);
