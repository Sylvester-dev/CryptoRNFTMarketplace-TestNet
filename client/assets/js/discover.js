...
$('#forSale').click(async ()=>{
  ...
  let activeCount = ifOfferDetailsDuplicatesRemoved.filter(item => !item.isSold && item.active).length;
  if(activeCount < 1){
    $('.minted-wrapper').html(`<div class="no-art-for-sale shadow-sm">There is currently no artwork for sale on CryptoR, but you can change that <a class="gradient-text" href="create.html"> here!<a> 😎<div>`);
  }
  recentlyPutForSale();
});
...
$('#notForSale').click(async ()=>{
  ...
  let inactiveCount = inactiveArtwork.filter(item => !item.active).length;
  if(inactiveCount < 1){
    $('.minted-wrapper').html(`<div class="no-art-for-sale shadow-sm">There is currently no artwork not for sale on CryptoR, but you can change that <a class="gradient-text" href="create.html"> here!<a> 😎<div>`);
  }
...
async function viewAll(){
  ...
  if(activeCount < 1 && inactiveCount < 1){
    $('.minted-wrapper').html(`<div class="no-art-for-sale shadow-sm">There is currently no artwork on CryptoR, but you can change that <a class="gradient-text" href="create.html"> here!<a> 😎<div>`);
  }
  recentlyPutForSale();
  recentlyMintedAndNotOnSale();
};
...
async function recentlySold(){
  try {
    ...
    for (i = 0; i < restrictedToRecentEightSalesArray.length; i++) {
      ...
      $('#soldCardButton' + tokenAddress + id).html(`<a href="http://localhost:8000/client/token.html?token=`+tokenAddress+id+`"><button class="btn btn-light view-btn">View</button></a>`);
...
function soldCardShareOptions(tokenAddress, id){
  ...
  let tokenPage = `http://localhost:8000/client/token.html?token=${tokenAddress+id}`;
  ...
  try{
    ...
    for (i = 0; i < ifOfferDetailsLength; i++) {
      if(ifOfferDetailsDuplicatesRemoved[i].active == true && ifOfferDetailsDuplicatesRemoved[i].isSold == false){
        ...
        $('#button' + tokenAddress + id).html(`<a href="http://localhost:8000/client/token.html?token=`+tokenAddress+id+`"><button class="btn btn-primary buy-btn">Buy</button></a>`);
      }
      darkmodeForDynamicContent();
    }
  } catch (error){
  ...
  try{
    ...
    for (i = 0; i < inactiveArtwork.length; i++) {
      if(inactiveArtwork[i].active == false){
        ...
        $('#button' + tokenAddress + id).html(`<a href="http://localhost:8000/client/token.html?token=`+tokenAddress+id+`"><button class="btn btn-light view-btn">View</button></a>`);
...
async function checkForApprovalBeforePuttingOnSale(tokenAddress, id){
  try{
    let approved = await CryptoRTokenInstance.methods.isApprovedForAll(user.attributes.ethAddress, CryptoRMarketplaceAddress).call();
...
function setApprovalIfNeeded(tokenAddress, id){
  $('#setApprovalBtn' + tokenAddress + id).click(async() =>{
    ...
    await CryptoRTokenInstance.methods.setApprovalForAll(CryptoRMarketplaceAddress, true).send({from: user.attributes.ethAddress}, (err, txHash) => {
      if(err){
        alert(err.message);
...
function putOnSale(tokenAddress, id, royalty, creator){
  $('#putOnSaleBtn' + tokenAddress + id).click(async()=>{
    ...
    try{
      await CryptoRMarketplaceInstance.methods.setOffer(amountInWei, id, tokenAddress).send({from: user.attributes.ethAddress});
...
function removeFromSale(tokenAddress, id, royalty, creator){
  ...
  $('#removeFromSaleBtn' + tokenAddress + id).click(async()=>{
    ...
    try{
      await CryptoRMarketplaceInstance.methods.removeOffer(id, tokenAddress).send({from: user.attributes.ethAddress});
...
function changePriceFrontEnd(tokenAddress, id, royalty, creator){
  $('#changePriceBtn' + tokenAddress + id).click(async()=>{
    ...
    try{
      await CryptoRMarketplaceInstance.methods.changePrice(amountInWei, id, tokenAddress).send({from: user.attributes.ethAddress});
...
function transferToken(tokenAddress, id){
  $('#transferTokenBtn' + tokenAddress + id).click(async()=>{
    ...
    try{
      await CryptoRTokenInstance.methods.safeTransferFrom(user.attributes.ethAddress, toAddress, id).send({from: user.attributes.ethAddress});
      ...
      $('#owner' + tokenAddress + id).attr('href', "http://localhost:8000/client/profile.html?address=" + toAddress);
                      ...
                      <div id="soldCard`+tokenAddress+id+`" class="card sold-item shadow-sm">
                        <div class="top-row">
                          <div class="creator-photo">
                            <a href="http://localhost:8000/client/profile.html?address=`+owner+`"><img loading="lazy" class="owner shadow-sm" id="soldCardOwnerPhoto`+tokenAddress+id+`" src="" width="40" alt="owner photo">
                        ...
                        <div class="embed-responsive embed-responsive-1by1">
                          <a href="http://localhost:8000/client/token.html?token=`+tokenAddress+id+`">
                        ...
                        <div class="card-body">
                          <a class="anchor" href="http://localhost:8000/client/token.html?token=`+tokenAddress+id+`">
...
function cardDiv(tokenAddress, id, owner){
  let nftCard = `<div id="nftCard`+tokenAddress+id+`" class="grid-helper col-xs-12 col-sm-6 col-md-4 col-lg-3 col-xl-3">
                    <div id="card`+tokenAddress+id+`" class="card minted-item shadow-sm">
                      <div class="top-row">
                        <div class="creator-photo">
                          <a id='owner`+tokenAddress+id+`' href="http://localhost:8000/client/profile.html?address=`+owner+`"><img loading="lazy" class="owner shadow-sm" id="ownerPhoto`+tokenAddress+id+`" src="" width="40" alt="owner photo">
