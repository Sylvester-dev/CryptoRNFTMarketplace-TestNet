...
async function mint(nftMetadataPath, nftPath){
  ...
  try {
    let receipt = await CryptoRTokenInstance.methods.createArtwork(nftMetadataPath, royalty).send({from: user.attributes.ethAddress});
    $('#saveToIPFSBtn').prop('disabled', true);
...
async function uploadToDB(tokenId, nftMetadataPath, nftPath){
  $('#saveToIPFSBtn').html(`Finishing upload to CryptoR <div class="spinner-border spinner-border-sm text-light" role="status">
                                                            <span class="sr-only">Loading...</span>
                                                          </div>`);
  try {
    ...
    artwork.set('tokenAddress', CryptoRTokenAddress.toLowerCase());
    ...
    if($('#onSaleSwitch').prop('checked')){
      ...
      $('#successfulText').html('<a href="http://localhost:8000/client/profile.html?address='+user.attributes.ethAddress+'">Click here to view.</a> <a href="erc-721.html">Or click here to make another one.</a>');
    }

  } catch (err) {
    console.log(err);
    alert("Error uploading to CryptoR");
    $('#saveToIPFSBtn').prop('disabled', false);
    $('#saveToIPFSBtn').html("Upload and Mint");
  }
};
